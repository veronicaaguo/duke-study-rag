...
- **`src/retrieval/vector_store.py`** — AI defaulted to OpenAI `text-embedding-3-small`, which caused ingestion to hang indefinitely on large corpora. Switched to local `all-MiniLM-L6-v2` via `SentenceTransformerEmbeddingFunction`; added env-var fallback logic and `tqdm` progress bar. Also fixed a variable shadowing bug where `embedding_model` (parameter) was used instead of `self.embedding_model` in the branch logic.
